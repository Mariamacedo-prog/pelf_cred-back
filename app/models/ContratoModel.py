...
class ContratoModel(Base):
    ...
    plano_id = Column(UUID(as_uuid=True), ForeignKey("planos.id"), nullable=True)
    anexos_list_id = Column(ARRAY(UUID(as_uuid=True)), nullable=True)
    servicos_list_id = Column(ARRAY(UUID(as_uuid=True)), nullable=True)
    ...
    plano = relationship("PlanoModel", back_populates="contrato")