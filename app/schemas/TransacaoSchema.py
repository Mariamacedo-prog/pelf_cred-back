...
class TransacaoResponseAtraso(BaseModel):
    id: Optional[UUID] = Field(default_factory=uuid4)
    contrato: Optional[ContratoResponse] = None
    comprovante_numero: Optional[str] = None
    status_parcela: Optional[str] = None
    meio_pagamento: Optional[str] = None
    status_comprovante: Optional[str] = None
...
class PaginatedTransacaoResponseAtraso(BaseModel):
    total_items: int
    total_paginas: int
    pagina_atual: int
    items: int
    offset: int
    data: List[TransacaoResponseAtraso]